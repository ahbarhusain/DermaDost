...
function scrollTo(hash) {
  ...
  if (nav.length > 1) {
    ...
    var top = 0;
    if (header.css("position") == "fixed") top = header.height();
...
function showFeed() {
  ...
  feed.load(function (result) {
    ...
    $.each(result.feed.entries, function () {
      console.log(this);
      var entry = $('<a class="feed-entry"/>')
        .attr("href", this.link)
        .append($('<div class="feed-icon pic-icon"/>'))
        .appendTo(cont);
...
function bindSignup() {
  $("#submit").click(function (e) {
    ...
    if ($.trim(name) == "" || $.trim(email) == "") return false;
...
function prev() {
  $(
    '<div style="background-color:rgba(0,0,0,.7);position:fixed;bottom:0;right:20px;color:#FFF;z-index:100;border-top-left-radius:5px;border-top-right-radius:5px;padding:3px 10px">Development Preview</div>'
  ).appendTo($("body"));
}
